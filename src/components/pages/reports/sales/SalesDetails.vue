      ...
      <div class="card-body">
        <div class="row">
          <div class="col-12 d-inline-flex">
            <div>
            <router-link :to="{ name: 'Sales Reports' }"  tag="span" class="button-tooltip pt-1 float-left">
        ...
        <div class="row">
          <div class="col-md-3 col-4 mb-1">
            ...
            <model-list-select class="form-control mb-1" :list="warehouseinfoList" v-model="dateEntity.warehouseOid" option-value="oid" optionText="warehouseName" placeholder="Select Warehouse Name" v-validate="'required'" name="Warehouse Name"/>
          ...
          <div class="col-md-3 col-4 mb-1">
            <label><b>RO / ARO Officer</b></label>
            <input class="form-control mb-1" :bootstrap-styling = "true" v-model="dateEntity.salesPerson" option-value="name" optionText="name" placeholder="RO / ARO Officer" name="Sales Person"/>
          </div>
          <div class="col-md-3 col-4 mb-1">
            <label> <b>Passport No.</b></label>
            <input class="form-control mb-1" v-model="dateEntity.passport" placeholder="Passport Number" name="Passport No."/>
          </div>
          <div class="col-md-3 col-4 mb-1">
      ...
      <div v-if="entity.length>0">       
        ...
        >
          <template slot="items" slot-scope="props">
            ...
            <td class="text-xs-left">{{ props.item.salesD }}</td>
            <td class="text-xs-left">{{ props.item.salesPerson }}</td>
            <td class="text-xs-left">{{ props.item.passport }}</td>
            <td class="text-xs-left">{{ props.item.holdername }}</td>
            <td class="text-xs-left">{{ props.item.billOfEntryNo }}</td>
            <td class="text-xs-left">{{ props.item.billOfEntryD }}</td>
            <td class="text-xs-left">{{ props.item.billOfEntryOfficeID }}</td>
        ...
        <div class="container">
          <div class="row">
            <div class="col-xs-2 d-flex justify-content-end">   
              <p><b>Total Sales Quantity: </b>{{totalSalesQuantity}}</p>
            </div>
          </div>
          <div class="row">
            <div class="col-xs-2 d-flex justify-content-end">
              <p><b>Total Sales Price: </b>{{totalSalesPrice}}</p>
            </div>
          </div>
        </div>
  ...
  data() {
    return {
      ...
      warehouseHeaders: [
        ...
        { text: "Sales Date", value: "salesD", align: "left" },
        { text: "Sales Person", value: "salesPerson", align: "left" },
        { text: "Passport", value: "passport", align: "left" },
        { text: "Holder Name", value: "holdername", align: "left" },
        { text: "Bill Of Entry No", value: "billOfEntryNo", align: "left" },
        { text: "Bill Of Entry Date", value: "billOfEntryD", align: "left" },
        { text: "Bill of Entry Office Id", value: "billOfEntryOfficeID", align: "left" },
  ...
  methods: {    
    ...
    getShow(){
      ...
      this.entity = []
          ...
          let req = constant.getRequest(url.operation_report_v1_get_sales_list_details_parameter, body);
          lumos_client.onPost(url.operation_report_v1_get_sales_list_details_parameter, req)
          .then(result => {
            ...
            this.totalSalesPrice = result.data.body.totalSalesPrice;
            this.totalSalesQuantity = result.data.body.totalSalesQuantity;
          }).catch(error =>{
          })
      });
    },    
    getPdf() {
      ...
      let body =  this.modifier(this.dateEntity); 
      let req = constant.getRequest(url.operation_report_v1_get_sales_list_details_export,body);
      lumos_client.onPost(url.operation_report_v1_get_sales_list_details_export, req)
        .then((result) => {
          this.progressBar.loading = false;
          this.progressBar.show = false;
          let res = result.data.header.responseCode;
