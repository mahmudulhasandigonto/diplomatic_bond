    ...
    <v-card flat>
      ...
      <div v-if="entity.length>0">
        ...
        >
          ...
          <!--nameOfHolder, passportNo, totalQuantity, totalValue, totalTax, totalPrice, invoiceDate-->
            <!--<td class="text-xs-left">{{ props.index + 1 }}</td>-->
            <td class="text-xs-left">{{ props.item.invoiceDate }}</td>
...
export default {
  ...
  data() {
    return {
      ...
      warehouseinfoList:[],
      warehouseHeaders: [
        // { text: "#", value: "", align: "left" },nameOfHolder, passportNo, totalQuantity, totalValue, totalTax, totalPrice, invoiceDate
        ...
        { text: "Total CIF Value($)", value: "totalValue", align: "left" },
  ...
  methods: {    
    ...
    getWarehouseList(){
            ...
            .then(result => {     
              ...
              } else if(localStorage.getItem("lumos_roleOid")=='reportchairman'){
                this.warehouseinfoList = [
                  {
                    "oid":"002",
                    "warehouseName":"Sabir Traders Limited"
                  }
                ]
                this.dateEntity.warehouseOid = '002'
    ...
    getShow(){
      ...
      this.entity = []
          ...
          let req = constant.getRequest(url.operation_report_v1_get_date_wise_invoice_parameter, body);
          lumos_client.onPost(url.operation_report_v1_get_date_wise_invoice_parameter, req)
          .then(result => {
    ...
    getPdf() {
      ...
      let body =  this.modifier(this.dateEntity);
      let req = constant.getRequest(url.operation_report_v1_get_date_wise_invoice_export, body);
      lumos_client.onPost(url.operation_report_v1_get_date_wise_invoice_export, req)
        .then((result) => {
          this.progressBar.loading = false;
          this.progressBar.show = false;
          let res = result.data.header.responseCode;
    ...
    getCSV() {
      ...
      let body =  this.modifier(this.dateEntity);
      let req = constant.getRequest(url.operation_report_v1_get_sales_list_export,body);
      lumos_client.onPost(url.operation_report_v1_get_sales_list_export, req)
        .then((result) => {
          this.progressBar.loading = false;
          this.progressBar.show = false;
          let res = result.data.header.responseCode;
