      ...
      <div v-if="entity.length>0">
        ...
        >
          <template slot="items" slot-scope="props">
            ...
            <td class="text-xs-left">{{ props.item.invoiceNo }}</td> 
  ...
  data() {
    return {
      ...
      warehouseHeaders: [
        ...
        { text: "Invoice No", value: "invoiceNo", align: "left" },
  ...
  methods: {    
    ...
    getShow(){
      ...
      this.entity = []
          ...
          let req = constant.getRequest(url.operation_report_v1_get_invoice_wise_sales_parameter, body);
          lumos_client.onPost(url.operation_report_v1_get_invoice_wise_sales_parameter, req)
          .then(result => {
    ...
    getPdf() {
      ...
      let body =  this.modifier(this.dateEntity);
      let req = constant.getRequest(url.operation_report_v1_get_invoice_wise_sales_export, body);
      lumos_client.onPost(url.operation_report_v1_get_invoice_wise_sales_export, req)
        .then((result) => {
          this.progressBar.loading = false;
          this.progressBar.show = false;
          let res = result.data.header.responseCode;
