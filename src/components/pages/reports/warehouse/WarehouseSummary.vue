       ...
       <!-- <div class="row">
        ...
        <div class="row">
          ...
          <div class="col-md-4 col-4 mb-1">
            ...
            <model-list-select class="form-control-sm" :list="warehouseList" v-model="dateEntity.warehouseOid" option-value="oid" optionText="warehouseName" placeholder="Select Warehouse Name" name="Warehouse Name"/>
          </div>
        </div>
      ...
      <div v-if="entity.length > 0">
        ...
        >
          <template slot="items" slot-scope="props">
            ...
            <td class="text-xs-left">{{ props.item.totalPurchasePieces }}</td>
            <td class="text-xs-left">{{ props.item.totalSalesPieces }}</td>
            <td class="text-xs-left">{{ props.item.totalStock }}</td>
            <td class="text-xs-left">{{ props.item.totalPurchasePrice }}</td>
            <td class="text-xs-left">{{ props.item.totalSalesPrice }}</td>
            <td class="text-xs-left">{{ props.item.totalActualSalesPrice }}</td>
  ...
  data() {
    return {
      ...
      warehouseHeaders: [
        ...
        { text: "Warehouse", value: "warehouseName", align: "left" },
        { text: "Allocation($)", value: "allocateValue", align: "left" },
        { text: "Total Purchase (Pics)", value: "totalPurchasePieces", align: "left" },
        { text: "Total Sell (Pics)", value: "totalPurchasePieces", align: "left" },
        { text: "Total Stock (Pics)", value: "totalStock", align: "left" },
        { text: "Total Purchase ($)", value: "totalPurchasePrice", align: "left" },
        { text: "Total Sales ($)", value: "totalSalesPrice", align: "left" },
        { text: "Total Actual Sell ($)", value: "totalActualSalesPrice", align: "left" },
        { text: "Remaining Balance ($)", value: "remainingBalance", align: "left" }
      ],
  ...
  methods: {
    ...
    getShow(){
      ...
      this.entity = []
          ...
          let req = constant.getRequest(url.operation_report_v1_get_warehouse_summary_parameter, body);
          lumos_client.onPost(url.operation_report_v1_get_warehouse_summary_parameter, req)
          .then(result => {
              this.progressBar.loading = false;
              this.progressBar.show = false;
    ...
    getPdf() {
      this.$validator.validateAll().then(result => {
        ...
        let body =  this.dateEntity;
        //let body = {};
        let req = constant.getRequest(url.operation_report_v1_get_warehouse_summary_export,body);
        lumos_client
          .onPost(url.operation_report_v1_get_warehouse_summary_export, req)
          .then((result) => {
            let res = result.data.header.responseCode;
            this.progressBar.loading = false;
