     ...
     <div class="">
        <div class="asset-top-panel">
            <div class="row col-12  nopadding">
                <router-link tag="button" class="btn btn-success ripple" style="background-color:#3f740d; border:none;" :to="{name:'New WarehouseInfo'}">
        ...
        <div class="">
            ...
            <template slot="items" slot-scope="props" >
                <td class="text-xs-left w-20">{{ props.item.warehouseNo }}</td>
                <td class="text-xs-left w-30">{{ props.item.warehouseName }}</td>
                <td class="text-xs-left w-15">{{ props.item.issueDate }}</td>
                <td class="text-xs-left w-15">{{ props.item.refLicenseNo }}</td>
                <td class="text-xs-left w-15">{{ props.item.refLicenseDate }}</td>
                <!-- <td class="text-xs-left w-10">{{ props.item.status }}</td> -->
                <td class="text-center w-5">
                    <router-link 
                    tag="span" 
                    :to="{name:'Update WarehouseInfo', params: { id: props.item.oid }}"
    ...
    data(){
        return{
            ...
            headers: [
                ...
                { text: "Issue Date", value: "issueDate", align: "left" },
                { text: "License No", value: "refLicenseNo", align: "left" },                
                { text: "License Date", value: "refLicenseDate", align: "left" },
