     ...
     <div class="">
        <div class="asset-top-panel">
            <div class="row col-12  nopadding">
                <router-link tag="button" class="btn btn-success ripple" style="background-color:#3f740d; border:none;" :to="{name:'New ProductDetails'}">
        ...
        <div class="">
            ...
            <template slot="items" slot-scope="props" >                
                <td class="text-xs-left w-30">{{ props.item.warehouseOid }}</td>
                <td class="text-xs-left w-35">{{ props.item.productOid }}</td>
                <td class="text-xs-left w-30">{{ props.item.productCode }}</td>
                <td class="text-center w-5">
                    <router-link 
                    tag="span" 
                    :to="{name:'Update ProductDetails', params: { id: props.item.oid }}"
...
export default {
    ...
    data(){
        return{
            entity:[],
            headers: [
                { text: "Warehouse Name", value: "warehouseOid", align: "left" },
                { text: "Product Name", value: "productOid", align: "left" },
